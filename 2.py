        ...
        for idx, (user, pwd) in enumerate(accounts, 1):
            ...
            with SB(
                ...
                try:
                    ...
                    time.sleep(3) # 给页面缓冲时间以加载可能的 CF 盾
                    
                    # === 新增：检测登录页的 CF 盾，并尝试 Tab+Space 锁定验证 ===
                    if self.is_turnstile_present(sb):
                        self.log("🛡️ 检测到登录页 CF 盾拦截，尝试模拟 Tab+Space 验证...")
                        try:
                            # 尝试点击背景聚焦，然后连续尝试 Tab + 空格进行验证
                            sb.click("body", timeout=2)
                            time.sleep(0.5)
                            actions = ActionChains(sb.driver)
                            # 模拟按一下 Tab 选定复选框，再按一下 Space 进行勾选
                            actions.send_keys(Keys.TAB).pause(0.5).send_keys(Keys.SPACE).perform()
                        except Exception as e:
                            self.log(f"模拟按键过程发生异常: {e}")
                        
                        # 调用原有的验证等待逻辑确保通过
                        if not self.wait_turnstile(sb, timeout=40):
                            self.log("⚠️ CF盾处理可能未完成，将继续尝试加载用户名输入框...")
                    # =======================================================
